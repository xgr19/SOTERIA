...
extern struct stream_list_indicate g_streamListHeader;
...
void* monitorProcess(void *arg) {
    ...
    uint32_t count = 0;
    fprintf(moniterFp, "Second,BloomFilter,RingBuffer,RB_Full_Count,DHASH_Buffer,FlowQueue_Buffer, PkgReceived,PkgProcced,PkgToMnn,StreamMnnCount\n");
    while (true) {
        ...
        fprintf(moniterFp, "%d,%lu,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%d\n", count, gBloomFilter.dwFilterSize, ringBufferLen * (MAX_RCV_PKT_BUF_LENGTH * sizeof(uint8_t)), gRingBufferFull
                ...
                , g_streamListHeader.size * (sizeof(struct stream_list_node) + MAX_SINGLE_MNN_LEN * MAX_MNN_STREAMS  + RCV_PKT_APPEND_HEAD_LEN * MAX_MNN_STREAMS)
