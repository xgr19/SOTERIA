...
void* packetRecv(void *arg) {
    ...
    rcv_size = 1024*1024*512;    /*Modify kernel param  sysctl -w net.core.rmem_max=*/
    optlen = sizeof(rcv_size);
    err = setsockopt(sock,SOL_SOCKET,SO_RCVBUF, (char *)&rcv_size, optlen);    
    if(err<0){
        DEBUGPRINTF("SO_RCVBUF set SO_RCVBUF is error\n");
    }

    err = getsockopt(sock, SOL_SOCKET, SO_RCVBUF,&rcv_size, &optlen);
    if(err<0){
        DEBUGPRINTF("SO_RCVBUF get SO_RCVBUF info error\n");
    }
    printf(">>> SO_RCVBUF:%d\n", rcv_size);

#endif
    timeval tv = {0, 100};
    err = setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(timeval));
    if(err < 0){
        DEBUGPRINTF("SO_RCVBUF set SO_RCVTIMEO is error\n");
    }
